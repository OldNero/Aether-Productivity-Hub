  ...
  reader.onload = async (e) => {
    ...
    try {
        ...
        if (err.message === 'DUPLICATE_IMPORT') {
            ...
            // Show the real Supabase error so it's visible in the UI for diagnosis
            const detail = err.message?.replace('SUPABASE_ERROR: ', '') || 'Unknown error';
            uiStore.showAlert('Import Failed', `Database error: ${detail}`, 'error');
        }
    } finally {
...
<<template>
  <div class="investments-view-container">
